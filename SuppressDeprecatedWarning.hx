...
class SuppressDeprecatedWarning {
	public static function run() {
		Context.onAfterTyping(function(_) {
			Context.filterMessages(x -> switch x {
				case Warning(msg, pos) if (~/src\/js\/node\/Url\.hx$/.match(Context.getPosInfos(pos).file)):
				...
				case Warning(msg, pos) if (~/src\/js\/node\/domain\/Domain\.hx$/.match(Context.getPosInfos(pos).file)):
